...
class _ChannelsTabState extends ConsumerState<ChannelsTab> {
  ...
  Future<void> _deleteChannelConfig(String channelId, String label) async {
    ...
    if (confirmed == true) {
      await ref.read(configProvider.notifier).updateChannels(<String, dynamic>{
        channelId: <String, dynamic>{'enabled': false, 'settings': <String, dynamic>{}},
      });
      if (mounted) {
        ...
        if (mounted) {
          AppSnackBar.showError(
            ...
            'settings.channels.tg_failed_save'.tr(
              namedArgs: {'error': (validation['message'] as String?) ?? 'Invalid token'},
            ),
          );
        }
        return;
        ...
        if (policy != 'disabled')
          AppSettingsInputField(
            controller: _getController(
              ...
              (settings['serviceAccountJsonPath'] as String?) ?? '',
            ),
        ...
        if (policy != 'disabled')
          AppSettingsInputField(
            controller: _getController(
              channelId,
              'subscriptionId',
              (settings['subscriptionId'] as String?) ?? '',
            ),
      ...
      inputFields.add(
        AppSettingsInputField(
          controller: _getController(
            channelId,
            'pairingCode',
            (settings['pairingCode'] as String?) ?? '',
          ),
        ...
        onEdit: () async {
          ...
          if (mounted) {
            setState(() {
              ...
              if (channelId == 'telegram') {
                final currentToken = (token ?? settings['botToken'] ?? '') as String;
                ...
                for (final field in [
                  ...
                  final String val = (settings[field] as String?) ?? '';
                  _getController(channelId, field, val).text = val;
                }
              } else {
                final currentToken = (token ?? settings['token'] ?? '') as String;
                _getController(channelId, 'token', currentToken).text =
                    currentToken;
              }

              if (dmPolicyStr == 'pairing') {
                final String code = (settings['pairingCode'] as String?) ?? '';
                _getController(
                  channelId,
                  'pairingCode',
                  code,
