...
class _IntegrationsTabState extends ConsumerState<IntegrationsTab> {
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        if (data.containsKey('web')) {
          clientId = (data['web'] as Map<String, dynamic>)['client_id'] as String?;
        ...
        if (data.containsKey('installed')) {
          clientId = (data['installed'] as Map<String, dynamic>)['client_id'] as String?;
          clientSecret = (data['installed'] as Map<String, dynamic>)['client_secret'] as String?;
        } else if (data.containsKey('web')) {
          // Fallback for some desktop-configured web IDs
          clientId = (data['web'] as Map<String, dynamic>)['client_id'] as String?;
          clientSecret = (data['web'] as Map<String, dynamic>)['client_secret'] as String?;
        } else if (data.containsKey('client_id')) {
          clientId = data['client_id'] as String?;
          ...
          AppSettingsInput(
            ...
            onEdit: () async {
              ...
              if (mounted) {
                setState(() {
                  if (kIsWeb) {
                    _getOAuthEditController('google_client_id_web').text = (creds['clientIdWeb'] as String?) ?? '';
                  } else {
                    _getOAuthEditController('google_client_id_desktop').text = (creds['clientIdDesktop'] as String?) ?? '';
                    _getOAuthEditController('google_client_secret').text = (creds['clientSecret'] as String?) ?? '';
                  }
  ...
  Future<void> _handleGoogleSignIn() async {
    ...
    try {
      final creds = await ref.read(configProvider.notifier).getGoogleCredentials();
      final clientId = (kIsWeb ? creds['clientIdWeb'] : creds['clientIdDesktop']) as String?;
