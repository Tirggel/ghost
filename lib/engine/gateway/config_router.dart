...
class ConfigRouter {
  ...
  void register() {
    ...
    gateway.rpcRegistry.register('config.get', (params, context) async {
      // Optimize: Only run migrations if not already marked as done in vault
      final migrationDone = await storage.get('migration_v1_done') == 'true';
      if (!migrationDone) {
        ...
        await storage.set('migration_v1_done', 'true');
      }
    ...
    // 21. Backup
    gateway.rpcRegistry.register('config.backup', (params, context) async {
      _log.info('System backup requested via RPC');
      final ghostDir = Directory(p.dirname(configPath));
      if (!await ghostDir.exists()) {
    ...
    gateway.rpcRegistry.register('config.restore', (params, context) async {
      ...
      try {
        ...
        // 1. Prepare for wipe
        await agentManager.ragMemoryEngine.close();
        ...
        // 4. Extract
        for (final file in archive) {
          var filename = p.normalize(file.name);
          if (filename.contains('..')) continue;

          // Strip common root prefix if found
          if (commonPrefix.isNotEmpty && filename.startsWith(commonPrefix)) {
          ...
          if (file.isFile) {
            ...
            final outFile = File(p.join(ghostDir.path, filename));
        ...
        try {
          ...
          if (await vaultJsonFile.exists()) {
            final raw = await vaultJsonFile.readAsString();
            final data = jsonDecode(raw) as Map<String, dynamic>;
            for (final entry in data.entries) {
              await storage.set(entry.key, entry.value.toString());
            }
            await vaultJsonFile.delete();
            _log.info('Vault secrets restored from backup zip');
          }
        } catch (e) {
        ...
        try {
          ...
          final passwordHash = restoredConfig.gateway.auth.passwordHash;
          final seedString =
  ...
  void _triggerGatewayRestart(String message) {
    gateway.broadcast('gateway.log', {
      'level': 'INFO',
      'message': '↺ $message',
