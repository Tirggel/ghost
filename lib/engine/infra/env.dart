...
class Env {
  ...
  static String get scriptsDir {
    ...
    final envDir = getGhost('SCRIPTS_DIR');
    if (envDir != null) return envDir;

    // 2. Try relative to CWD (usually works in dev if running from root)
    final cwdScripts = Directory(p.join(Directory.current.path, 'scripts'));
    if (cwdScripts.existsSync()) return cwdScripts.path;

    // 3. Try relative to Platform.script (standard for compiled/snapshot runs)
    // We go up 2 levels from lib/main.dart or bin/exe
    final scriptPath = Platform.script.toFilePath();
    final exeDir = p.dirname(p.dirname(scriptPath));
    final exeScripts = Directory(p.join(exeDir, 'scripts'));
    if (exeScripts.existsSync()) return exeScripts.path;

    // 4. Last ditch effort: look for a known project path if we are peter
    if (Platform.environment['USER'] == 'peter') {
      final peterPath = '/home/peter/Developer/flutter-dev/ghost/scripts';
      if (Directory(peterPath).existsSync()) return peterPath;
    }

    // Fallback to the old logic
    return p.join(exeDir, 'scripts');
  }
}
